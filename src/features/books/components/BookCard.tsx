...
export function BookCard({
  ...
  return (
    <Card
      noPadding
      hoverable
      className={cn("flex flex-col gap-4 p-4 sm:flex-row", className)}
    >
      {/* Cover */}
      <div className="relative aspect-[2/3] w-full shrink-0 overflow-hidden rounded-lg bg-zinc-100 sm:w-28 dark:bg-zinc-800">
        {coverUrl ? (
          <Image
            src={coverUrl}
          ...
          <div className="flex h-full w-full items-center justify-center">
            <BookOpen
              className="h-8 w-8 text-amber-700/40 dark:text-amber-500/40"
              aria-hidden="true"
            />
          </div>
        )}
